...
import mytimm
from mytimm import list_models, create_model, set_scriptable, has_model_default_key, is_model_default_key, \
    get_model_default_value
from mytimm.models.fx_features import _leaf_modules, _autowrap_functions
...
def test_model_default_cfgs(model_name, batch_size):
    ...
    if all([x <= MAX_FWD_OUT_SIZE for x in input_size]) and \
        ...
        if not isinstance(model, mytimm.models.MobileNetV3) and not isinstance(model, mytimm.models.GhostNet):
