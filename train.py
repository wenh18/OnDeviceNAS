...
def main():
    ...
    # subnets = []
    # subnetchoices = []
    # for i in range(2 * (int(512 / args.batch_size * 2503) + 1) * args.epochs):
    #     subnet, subnetchoice = model.generate_random_subnet(stage=3)
    ...
    # # print('dumped')
    # print("created")
    # np.save('subnets.npy', subnets)
    # np.save('subnetchoices.npy', subnetchoices)
    # print("saved")
    # subnets = np.load('subnets.npy')
    # subnetchoices = np.load('subnetchoices.npy')
    # print(subnets.shape[0])
    # print('loaded')
    # with open("randomsubnets.yaml", "r") as f:
    #     content = f.read()
    # file = open('randomsubnets.yaml', 'r', encoding="utf-8")
    # subnets = yaml.safe_load(file)
    # print(type(subnets), len(subnets), len(subnets[0]))
    # with open("randomsubnetchoices.yaml", "r") as f:
    #     content = f.read()
    # subnetchoices = yaml.load(content)
    # print(type(subnetchoices), len(subnetchoices), len(subnetchoices[0]))

    teachermodel = timm.create_model('mobilenetv3_large_100', pretrained=True)
    teachermodel.eval()
    teachermodel.cuda()
    # import pdb;pdb.set_trace()
    model, teachernet_map = load_to_MultiModel(model, "pretrainedweight/mobilenetv3_large_100_ra-f55367f5.pth")
    # model.bn1.eval()
    # model.multiblocks[0][0].bn1.eval()
    # model.multiblocks[0][0].bn2.eval()
    # for i in range(1, 15):
    #     exec("model.multiblocks[%s][0].bn1.eval()" % i)
    #     exec("model.multiblocks[%s][0].bn2.eval()" % i)
    #     exec("model.multiblocks[%s][0].bn3.eval()" % i)
    # model.multiblocks[15][0].bn1.eval()

    # model = load_to_MultiModel(model, "nostagestage_3epoch9.pth")
    ##################################for cifar
    # model.classifier =  Linear(in_features=1280, out_features=10, bias=True)
    #
    # fan_out = model.classifier.weight.size(0)  # fan-out
    # fan_in = 0
    # # if 'routing_fn' in n:
    # fan_in = model.classifier.weight.size(1)
    # init_range = 1.0 / math.sqrt(fan_in + fan_out)
    # nn.init.uniform_(model.classifier.weight, -init_range, init_range)
...
#model.generate_random_subnet(stage)
            ...
            for epoch in range(start_epoch, num_epochs):
                ...
                train_metrics = train_one_epoch(
                    ...
                    # subnets[epoch * int(1281167 / args.batch_size):(epoch + 1) * int(1281167 / args.batch_size), :], subnetchoices[epoch * int(1281167 / args.batch_size):(epoch + 1) * int(1281167 / args.batch_size), :],
                ...
                eval_metrics = validate(model, loader_eval, validate_loss_fn, args,
                                        ...
                                        teacher_model=teachermodel, stage=stage, is_distributed=False, writer=writer, epoch=epoch)
                ...
                if (epoch + 1) % args.savemodelfreq == 0:
                    name = "dividedentropy_" + str(stage) + "epoch" + str(epoch) + ".pth"
                    emaname = "dividedentropyema_" + str(stage) + "epoch" + str(epoch) + ".pth"
...
def train_one_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        with amp_autocast():
            # if stage == 3:
            #     # subnet, subnetchoice = subnets[batch_idx], subnetchoices[batch_idx]#
            #     subnet, subnetchoice = model.generate_random_subnet(stage)
            # else:
            #     subnet, subnetchoice = model.generate_random_subnet(stage)
            # print(subnet, subnetchoice)
            ...
            if stage < 3:
                ...
                for i in range(len(features)):
                    idx1, idx2 = teachernet_map[feature_idx_outs[i]-1][0], teachernet_map[feature_idx_outs[i]-1][1]
                    teacher_output = get_teacher_outputv2(input, teacher_model, idx1, idx2)
                    loss += torch.nn.MSELoss()(teacher_output, features[i])
                    del teacher_output
                loss /= len(features)
    ...
    with torch.no_grad():
        for batch_idx, (input, target) in enumerate(loader):
            ...
            with amp_autocast():
                ...
                # else:
                #     subnet, subnetchoice = model.generate_random_subnet(stage)
                # subnet = [0 for i in range(16)]
                # subnetchoice = [0 for i in range(16)]
                # else:
                #     subnet, subnetchoice = subnets[batch_idx], subnetchoices[batch_idx]#model.generate_random_subnet(stage)
                # print(subnet)
