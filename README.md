...
* Add ConvNeXT models /w weights from official impl (https://github.com/facebookresearch/ConvNeXt), a few perf tweaks, compatible with mytimm features
...
### Aug 18, 2021
* Optimizer bonanza!
  * Add LAMB and LARS optimizers, incl trust ratio clipping options. Tweaked to work properly in PyTorch XLA (tested on TPUs w/ `mytimm bits` [branch](https://github.com/rwightman/pytorch-image-models/tree/bits_and_tpu/mytimm/bits))
...
### April 1, 2021
* Add snazzy `benchmark.py` script for bulk `mytimm` model benchmarking of train and/or inference
* Add Pooling-based Vision Transformer (PiT) models (from https://github.com/naver-ai/pit)
  ...
  * Some `mytimm` cleanup/style tweaks and weights have hub download support
...
* Integrate [Hugging Face model hub](https://huggingface.co/models) into mytimm create_model and default_cfg handling for pretrained weight and config sharing (more on this soon!)
...
Py**T**orch **Im**age **M**odels (`mytimm`) is a collection of image models, layers, utilities, optimizers, schedulers, data-loaders / augmentations, and reference training / validation scripts that aim to pull together a wide variety of SOTA models with ability to reproduce ImageNet training results.
...
My current [documentation](https://rwightman.github.io/pytorch-image-models/) for `mytimm` covers the basics.

[mytimmdocs](https://fastai.github.io/mytimmdocs/) is quickly becoming a much more comprehensive set of documentation for `mytimm`. A big thanks to [Aman Arora](https://github.com/amaarora) for his efforts creating mytimmdocs.

[paperswithcode](https://paperswithcode.com/lib/mytimm) is a good resource for browsing the models within `mytimm`.
