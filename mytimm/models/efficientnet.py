...
class EfficientNet(nn.Module):
        ...
        for stage_idx in range(len(self.blocks)):
            for block_idx in range(len(self.blocks[stage_idx])):
                ...
                if multi_block_idx <= block_len - 2:
                    ...
                    if hasattr(self.blocks[stage_idx][block_idx], 'conv_dw'):  # inverted residual
                        distill_next.conv_dw = create_conv2d(
                            self.blocks[stage_idx][block_idx].mid_chs, self.blocks[stage_idx][block_idx].mid_chs, 3,
                            stride=stride, dilation=self.blocks[stage_idx][block_idx].dilation,
    ...
    def get_next_next_block(self, stage_idx, block_idx):
        if block_idx < (len(self.blocks[stage_idx]) - 2):
            return self.blocks[stage_idx][stage_idx + 2]
    ...
    def get_block_info(self, block):
        ...
        elif hasattr(block, 'conv_dw'):  # invertedresidual
