...
All of the models in `mytimm` have consistent mechanisms for obtaining various types of features from the model for tasks besides classification.
...
import torch
import mytimm
m = mytimm.create_model('xception41', pretrained=True)
...
import torch
import mytimm
m = mytimm.create_model('resnet50', pretrained=True, num_classes=0, global_pool='')
...
import torch
import mytimm
m = mytimm.create_model('densenet121', pretrained=True)
o = m(torch.randn(2, 3, 224, 224))
print(f'Original shape: {o.shape}')
...
import torch
import mytimm
m = mytimm.create_model('resnet50', pretrained=True, num_classes=0)
o = m(torch.randn(2, 3, 224, 224))
...
import torch
import mytimm
m = mytimm.create_model('ese_vovnet19b_dw', pretrained=True)
o = m(torch.randn(2, 3, 224, 224))
print(f'Original shape: {o.shape}')
m.reset_classifier(0)
o = m(torch.randn(2, 3, 224, 224))
...
`mytimm` allows a consistent interface for creating any of the included models as feature backbones that output feature maps for selected levels. 
...
import torch
import mytimm
m = mytimm.create_model('resnest26d', features_only=True, pretrained=True)
o = m(torch.randn(2, 3, 224, 224))
for x in o:
  print(x.shape)
```
...
import torch
import mytimm
m = mytimm.create_model('regnety_032', features_only=True, pretrained=True)
...
import torch
import mytimm
m = mytimm.create_model('ecaresnet101d', features_only=True, output_stride=8, out_indices=(2, 4), pretrained=True)
print(f'Feature channels: {m.feature_info.channels()}')
