...
```python
import mytimm
model = mytimm.create_model('tf_efficientnet_b0', pretrained=True)
model.eval()
```
...
```python
model = mytimm.create_model('tf_efficientnet_b0', pretrained=True, num_classes=NUM_FINETUNE_CLASSES)
```
