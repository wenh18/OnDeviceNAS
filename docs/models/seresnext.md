...
```python
import mytimm
model = mytimm.create_model('seresnext26d_32x4d', pretrained=True)
model.eval()
```
...
```python
model = mytimm.create_model('seresnext26d_32x4d', pretrained=True, num_classes=NUM_FINETUNE_CLASSES)
```
