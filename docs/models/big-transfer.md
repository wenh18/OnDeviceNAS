...
```python
import mytimm
model = mytimm.create_model('resnetv2_101x1_bitm', pretrained=True)
model.eval()
```
...
```python
model = mytimm.create_model('resnetv2_101x1_bitm', pretrained=True, num_classes=NUM_FINETUNE_CLASSES)
```
